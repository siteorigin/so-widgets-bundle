...
<div class="sow-taxonomy">
	...
	<?php if ( $display_format == 'text' || $display_format == 'links' ) echo '<p>' ?>
		...
		<?php foreach ( get_the_terms( $post->ID, $taxonomy_name ) as $term ) : ?>
			...
			<?php else: ?>
				<a class="so-taxonomy-<?php echo esc_attr( $display_format )?>" href="<?php echo get_term_link( $term, $taxonomy_name ) ?>" rel="tag"  <?php if ( ! empty( $new_window ) ) echo 'target="_blank" rel="noopener"'; ?>><?php echo esc_html( $term->name ) ?></a>
			<?php endif; ?>
		<?php endforeach; ?>
